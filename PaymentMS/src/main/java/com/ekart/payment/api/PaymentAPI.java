...
public class PaymentAPI {
	...
	@CircuitBreaker(name = "orderService", fallbackMethod = "payForOrderFallback")
	...
	public ResponseEntity<String> payForOrder(
		...
		OrderDTO orderDTO = template.getForEntity("http://CustomerMS/Ekart/customerorder-api/order/"+transactionDTO.getOrder().getOrderId(), OrderDTO.class).getBody();
		...
		Integer transactionId = paymentService.addTransaction(transaction);
		paymentCircuitBreakerService.updateOrderAfterPayment(transaction.getOrder().getOrderId(), transaction.getTransactionStatus().toString());
		String msg = environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_ONE")+" "+transaction.getTotalPrice()
		+" "+environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_TWO")+" "+transaction.getOrder().getOrderId()
